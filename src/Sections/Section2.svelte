<script>
  ...
  const vlineNumbers = [...Array(13).keys()].map(d => (d / 2 + 1));
  ...
  $: {
    if (width !== undefined && height !== undefined) {
      ...
      xScale = d3.scaleLinear().domain([1,7]).range([margin.l, width - margin.r])
      yScale = d3.scaleLinear().domain([0,0.8]).range([height / 2 - margin.b, margin.t])
        ...
        <svg {width} {height}>
          <g class="x-axis">
            {#each vlineNumbers as n}
              <line
                ...
                x1="{xScale(n)}"
                y1="{index <= 1 ? yScale(0) : yScale(0) + height / 3}"
                x2="{xScale(n)}"
                y2="{150}"
                stroke-width="0.3"
                stroke="white"
              ...
              {#if width > 467}
                <text
                  class="x-axis-ticks"
                  x="{xScale(n) + margin.l}"
                  y="{index <= 1 ? yScale(0) + 30 : yScale(0) + 30 + height / 3}"
                  fill="white"
                  text-anchor="middle"
                >{d3.format(",.1f")(n)}
                </text>
          ...
          {#if width > 467}
            <g class="x-axis-label">
              <text
                  x={width / 2 + margin.l}
                  y={index <= 1 ? height / 3 + height / 5 + 30 : height - margin.b - height / 10 + 30}
                  text-anchor="middle"
                  fill="gray"
              >
              ...
              <text class="axis-label"
                  x={width - margin.r / 2}
                  y={index <= 1 ? height / 3 + height / 5 : height - margin.b - height / 10}
                  text-anchor="end"
              >
                  More delays per day →
              </text>
          ...
          {:else}
            <g class="x-axis-label">
              <text
                  x={width / 2 + margin.l}
                  y={index <= 1 ? height / 3 + height / 5 - 130 : height - margin.b - height / 10 - 130}
                  text-anchor="middle"
                  fill="gray"
              >
                % of delayed flights, adjusted
              ...
              <text class="axis-label"
                  x={width - margin.r / 2}
                  y={index <= 1 ? height / 3 + height / 5 - 110 : height - margin.b - height / 10 - 110}
                  text-anchor="end"
              >
                  More delays per day →
              </text>
          ...
          <g class="y-axis-label">
            <text
                transform={`translate(${width > 467 ? 80 : 30}, ${index <= 1 ? height / 3 - 60 : height / 2 - 60}) rotate(-90)`}
                text-anchor="middle"
                fill="gray">
                Density
            </text>
                ...
                {#each texts as text}
                  <g class="chart-subtitle">
                    <text
                      ...
                      {index <= 1 ? 85 : text === "Normal times" ? 200 : height / 3 + 200})"
                      fill="white"
                      opacity={index <= 1 ? 0 : 1}
                    >
                      {text}
